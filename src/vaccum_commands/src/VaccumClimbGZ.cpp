...
rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr feedback_publisher;
...
#define LEFT_ARM_HOME_POSITION_BODY 0.0//radian
#define RIGHT_ARM_HOME_POSITION 0.0//radian
#define RIGHT_ARM_HOME_POSITION_BODY 0.0 //radian
...
std::vector<std::vector<double>> desired_goals = {
  {LEFT_ARM_HOME_POSITION, LEFT_ARM_HOME_POSITION_BODY, RIGHT_ARM_HOME_POSITION, RIGHT_ARM_HOME_POSITION_BODY},  // Home position first
  {-1.5,  1.5,  -1.5,  1.5},
  {-2.1,  2.0,  -2.1,  2.0},
  { 0.0,  2.0,  -2.1,  2.0},
  { 0.0,  6.28, -2.1,  2.0},
  ...
  const std::shared_ptr<const control_msgs::action::FollowJointTrajectory::Feedback> feedback)
{
  ...
  if (feedback_publisher) {
    ...
    error_msg.data = feedback->error.positions;
...
int main(int argc, char * argv[])
{
  ...
  feedback_publisher = node->create_publisher<std_msgs::msg::Float64MultiArray>("/feedback", 10);
  ...
  for (size_t i = 0; i < joint_names.size(); ++i) {
    ...
    goal_msg.path_tolerance[i].position = 0.5;  // 0.1 rad tolerance during path
    ...
    goal_msg.goal_tolerance[i].position = 0.5;  // 0.01 rad (~0.57 deg) at goal
